...
from open_deep_research.utils import extract_answer_text, get_api_key_for_model
from sales_outreach.configuration import LeadLoaderType, SalesConfiguration
...
# launched from src/sales_outreach/. Mirrors LINKEDIN_SESSION_PATH in open_deep_research.
...
def _build_lead_loader(loader_type: str, sheet_id: str | None, base_id: str | None, table_name: str | None):
    ...
    if loader_type == LeadLoaderType.AIRTABLE.value:
        from sales_outreach.tools.leads_loader.airtable import AirtableLeadLoader
    ...
    if loader_type == LeadLoaderType.HUBSPOT.value:
        from sales_outreach.tools.leads_loader.hubspot import HubSpotLeadLoader
        return HubSpotLeadLoader()

    from sales_outreach.tools.leads_loader.google_sheets import GoogleSheetLeadLoader
...
def get_docs_manager():
    ...
    """
    from sales_outreach.tools.google_docs_tools import GoogleDocsManager
