...
from orchestrator.graph import _isolated, finish_target, next_target
...
def _finish(failed):
    ...
    with patch("orchestrator.graph.get_lead_loader"):
...
def test_finish_failure_still_advances_and_resets():
    ...
    from orchestrator.graph import _isolated_finish
    from orchestrator.state import PER_TARGET_FIELDS
...
def test_reset_constant_covers_every_per_target_field():
    ...
    from orchestrator.graph import PER_TARGET_RESET
    from orchestrator.state import PER_TARGET_FIELDS
