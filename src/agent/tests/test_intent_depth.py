...
import pytest
from agent.graph import build_unified_agent
...
def _run(intent):
    ...
    async def fake_invoke_llm(system_prompt, user_message, model_name, config, response_format=None):
        if response_format is not None:
            if response_format.__name__ == "ResearchSufficiency":
                return response_format(sufficient=True, gaps="")
            calls["materials"] += 1
            return response_format(subject="subject", email="body")
        if "scorer" in system_prompt.lower() or "score" in system_prompt.lower()[:200]:
            calls["score"] += 1
            return "9.0"
        calls["materials"] += 1
    ...
    with patch("agent.graph.invoke_llm", side_effect=fake_invoke_llm), \
         patch("agent.graph.GmailTools", _Gmail), \
         patch("agent.graph.save_reports_locally"), \
         patch("agent.graph.resolve_targets", new=AsyncMock(return_value=[target])):
        config = {
            "recursion_limit": 100,
