...
3. **The entity-type layer itself** — `classify_entity_type` + Entity Schema Registry — validated as the right design (company-research-agent is proof it works for one entity type), but still 0% built. Everything else on the improvement list assumes generic research; this is what makes research *specific*.
