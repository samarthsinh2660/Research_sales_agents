...
- **Unified agent** (`src/agent/`): one entry point where the prompt names the subject — a company, a Google Sheet link, an inline list, or an event page to extract sponsors and speakers from — and an `intent` config decides how deep to go (`research` → `qualify` → `draft` → `send`). Research is nested as a subgraph, so its internals stay visible in traces. Sending pauses for explicit approval. See [`docs/unified-agent-design.md`](docs/unified-agent-design.md).
